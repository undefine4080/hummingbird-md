...
基于 [Shiki](https://shiki.style/) 的精确语法高亮，和 VS Code 编辑器使用同款 TextMate 语法引擎。亮色 / 暗色主题自动切换，代码块 hover 显示复制和全屏预览按钮。

### 目录导航

侧边栏自动生成文档目录树。点击标题跳转定位，阅读时实时高亮当前位置。切换阅读 tab 时自动刷新目录并恢复高亮。长文档的好帮手。

### Mermaid 图表

流程图、时序图、甘特图、类图……用 Mermaid 语法写的图表直接渲染出来，点击可全屏查看，支持缩放、平移、旋转和下载为 JPG。
...
已打开的文档再次触发预览时会自动跳转到已有预览 tab，不会重复创建。源文件内容改动后预览会自动刷新并保持当前阅读位置。
